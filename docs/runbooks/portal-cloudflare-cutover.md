...
Operational runbook for migrating an EXAMPLE-style brand's Portal from a
...
(EXAMPLE first; Example and any white-label brands follow once EXAMPLE has
been stable for 30 days).

## Prerequisites checklist

- [ ] **Cloudflare account** has the target zone (e.g. `example.com`) and
...
- [ ] **cdn.example.com** Worker is live and caching public reads (per
      `spec-api-cache-and-revalidation.md`). Verify with:
      ```bash
      curl -sI https://cdn.example.com/api/portal/events \
        | grep -i cache-status
      ```
...
wrangler r2 bucket create gatewaze-portal-example-assets
wrangler r2 bucket create gatewaze-portal-example-staging-assets
...
dig +short staging.example.com | head -1

# 200 OK from the Worker?
curl -sI https://staging.example.com/ | head -1

# Edge cache warms on the second request?
curl -sI https://staging.example.com/events/upcoming -o /dev/null -w "%{http_code}\n"
curl -sI https://staging.example.com/events/upcoming | grep -i cf-cache-status
...
for host in example.com my-conference.example.com; do
  echo "=== ${host} ==="
  curl -sI -H "Host: ${host}" https://staging.example.com/api/health | head -1
  curl -sI -H "Host: ${host}" https://staging.example.com/events | head -1
done
```
...
for path in / /events /events/upcoming /events/past; do
  curl -s -H "Host: example.com" "https://staging.example.com${path}" -o /dev/null -w "%{http_code} ${path}\n"
done

# 3. Verify the production Worker responds on its own URL (before DNS swap):
curl -sI -H "Host: example.com" "https://gatewaze-portal-example.<your-account>.workers.dev/" | head -1
```
...
# In the EXAMPLE Cloudflare zone:
#   example.com (A or CNAME) → previously Linode LB IP
...
# At this point, requests to example.com land on the Worker.
# Verify:
curl -sI https://example.com/ | head -1
curl -sI https://example.com/events | head -1
...
# similar). Watch with:
kubectl -n example get pods -w -l app=portal
...
Keep the deployment definition in values-example.yaml indefinitely. Re-
...
Per spec §13:
- **EXAMPLE**: first cutover. Soak ≥30 days before next brand.
- **Example**: follows EXAMPLE, same playbook. Different zone + R2 bucket
