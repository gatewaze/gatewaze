...
const response = await fetch('/functions/v1/user-signup', {
  ...
  body: JSON.stringify({
    ...
    user_metadata: {
      ...
      utm_campaign: 'demo2024_reminders'
    }
  })
});
```
...
```
https://cohorts.example.com/auth?utm_source=linkedin&utm_campaign=ai_summit&referral=abc123
```
...
```bash
curl -X POST https://db.example.com/functions/v1/user-signup \
