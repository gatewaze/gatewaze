--------------------------------------------------------------------------------
-- 00045: people_get_authenticated_sorted — let the broad search use trigram indexes
--------------------------------------------------------------------------------
-- 00044 added pg_trgm GIN indexes so the People list search stops full-scanning
-- the (now ~158k row) people table. The broad-search predicate still forced a
-- sequential scan, though: its company branch was
--   COALESCE(c.attributes->>'company', '') ILIKE $1
-- and the trigram index is on the bare  c.attributes->>'company'  expression
-- (bare is what the "company:" field search and the prospects list also query).
-- A single un-indexable OR branch makes the planner seq-scan the whole OR, so
-- the search timed out even with the indexes present.
--
-- Dropping the COALESCE is behaviour-preserving for search: when company is
-- NULL, `NULL ILIKE '%term%'` is NULL, which is false inside the OR — exactly
-- what COALESCE-to-'' then matching a non-empty term already did. The name
-- branch keeps its COALESCE (a NULL part would null the whole concatenation and
-- drop the row); idx_people_fullname_trgm indexes that exact COALESCE concat, so
-- it stays index-backed. Result: BitmapOr across the email / full-name / company
-- trigram indexes instead of a parallel seq scan.
--------------------------------------------------------------------------------
...
BEGIN
  ...
  IF v_field_expr IS NOT NULL AND v_value IS NOT NULL AND v_value <> '' THEN
    -- Field-scoped search. No COALESCE wrapper: v_value is non-empty here, so a
    -- NULL field yields NULL ILIKE (false in the WHERE) — same result as
    -- COALESCE-to-'' — while letting the bare expression match its trigram index
    -- (email/first_name/last_name/company/job_title/full-name concat from 00044).
    v_search := '%' || v_value || '%';
    v_predicate := format($f$%s ILIKE $1$f$, v_field_expr);
  ELSE
    -- Broad search across email / name / company (original behaviour). Each
    -- branch is written to match a trigram index from 00044 exactly so the
    -- planner can BitmapOr them instead of sequential-scanning:
    --   c.email                                  -> idx_people_email_trgm
    --   COALESCE(first,'')||' '||COALESCE(last,'') -> idx_people_fullname_trgm
    --   c.attributes->>'company'                 -> idx_people_company_trgm
    ...
    v_predicate := $f$c.email ILIKE $1
            ...
            OR c.attributes->>'company' ILIKE $1$f$;
  END IF;
...
COMMENT ON FUNCTION public.people_get_authenticated_sorted(integer, integer, text, text, text)
  IS 'Paginated, sortable, searchable listing of authenticated people with total count. Search supports "field:value" syntax (email, first_name, last_name, name, company, job_title, city, country, linkedin_url) and falls back to broad email/name/company matching. Predicate branches match the 00044 trigram indexes so search is index-backed.';
