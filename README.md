...
Gatewaze is a full-featured platform for managing events, calendars, registrations, and members. It provides both an admin interface for organizers and a public portal for attendees, with a modular architecture designed for extensibility.
...
- **Member Management** -- Manage member profiles, organizations, and membership tiers.
- **Registration** -- Handle event registrations with configurable forms, approval workflows, and attendee tracking.
- **Public Event Portal** -- A fast, SEO-friendly public site for browsing events, viewing calendars, and registering.
...
- **Module System** -- Extend functionality through self-contained modules that plug into the core platform.
...
The full stack runs with a single command. Database migrations are applied automatically on first startup.
...
# Edit docker/.env — at minimum, set real values for:
...
Wait for all containers to become healthy (first startup takes ~2 minutes for database initialization):
...
```bash
cp docker/.env.example docker/.env
# Set SUPABASE_URL, ANON_KEY, SERVICE_ROLE_KEY to your cloud project values
