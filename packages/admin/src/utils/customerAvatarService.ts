...
export class CustomerAvatarService {
  ...
  /**
   * Upload an avatar image for a customer
   */
  static async uploadAvatar(
    ...
    try {
      ...
      const { error: updateError } = await supabase.rpc('update_customer_avatar', {
  ...
  static async downloadGravatar(
    ...
    return this.downloadAndStoreAvatar(customerId, gravatarUrl, 'gravatar');
  }
  ...
  /**
   * Delete avatar from storage and clear customer record
   */
  static async deleteAvatar(
    ...
    try {
      ...
      const { error: updateError } = await supabase.rpc('clear_customer_avatar', {
        p_customer_id: customerId
      });
  ...
  static async syncCustomerAvatar(
    ...
    try {
      ...
      if (hasGravatar) {
        const result = await this.downloadGravatar(customerId, email);
        if (result.success) {
