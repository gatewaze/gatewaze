...
import { uploadBlogImage, deleteBlogImage, updateBlogImage, extractImagePath, validateImageFile } from '@/utils/imageUpload';
import LoadingSpinner from '@/components/shared/LoadingSpinner';

interface ImageUploadProps {
  value?: string;
  onChange: (url: string | null, path?: string | undefined) => void;
...
export const ImageUpload: React.FC<ImageUploadProps> = ({
  ...
  const handleFileUpload = useCallback(async (file: File) => {
    ...
    try {
      ...
      const currentImagePath = value ? extractImagePath(value) : null;

      let result;
      if (currentImagePath) {
        result = await updateBlogImage(currentImagePath, file);
      } else {
      ...
      if (result.success && result.url) {
        ...
        onImagePathChange?.(result.path || undefined);
  ...
  }, [value, onChange, onImagePathChange, maxSizeInMB, disabled]);
  ...
  }, [urlInput, onChange, onImagePathChange]);

  const handleRemoveImage = useCallback(async () => {
    if (disabled) return;

    const imagePath = value ? extractImagePath(value) : null;
    ...
    onImagePathChange?.(undefined);
    setUrlInput('');
  }, [value, onChange, onImagePathChange, disabled]);
