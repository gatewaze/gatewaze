...
export const brands: Record<string, BrandConfig> = {
  acme: {
    ...
    features: {
      ...
      warehouse: true,
      newsletters: true,
      slack: false, // Acme doesn't need Slack integration
    },
    ...
    api: {
      baseUrl: import.meta.env.VITE_API_BASE_URL
        || (import.meta.env.DEV ? 'http://api-acme.localhost' : 'https://api.tech.tickets'),
      scraperApiUrl: import.meta.env.VITE_SCRAPER_API_URL
        || (import.meta.env.DEV ? 'http://api-acme.localhost/api/scrapers' : 'https://api.tech.tickets/api/scrapers'),
    },
    ...
    ui: {
      ...
      logoPath: '/theme/acme/logo.svg',
      faviconPath: '/theme/acme/',
    },
  ...
  demo: {
    id: 'demo',
    name: 'Demo Community',
    title: 'Demo Community Admin',
    appName: 'Demo Community Dashboard',

    features: {
      competitions: false, // Demo doesn't need competitions
      discounts: false,    // Demo doesn't need discounts
      scrapers: true,     // Scrapers disabled until backend is configured
      blog: true,         // Demo doesn't need blog
      offers: true,
      members: true,       // Demo has members
      events: true,        // Events feature enabled for Demo
      jobs: true,          // Demo-specific feature
      cohorts: true,       // Demo-specific feature
      stripe: true,        // Stripe payments enabled for Demo
    ...
    supabase: {
      url: import.meta.env.VITE_SUPABASE_URL || 'https://demosupabase.supabase.co', // Placeholder - needs real URL
      anonKey: import.meta.env.VITE_SUPABASE_ANON_KEY || 'demo-anon-key-placeholder', // Placeholder - needs real key
      serviceRoleKey: import.meta.env.SUPABASE_SERVICE_ROLE_KEY,
    },

    api: {
      baseUrl: import.meta.env.VITE_API_BASE_URL
        || (import.meta.env.DEV ? 'http://api-demo.localhost' : 'https://api.example.com'),
      scraperApiUrl: import.meta.env.VITE_SCRAPER_API_URL
        || (import.meta.env.DEV ? 'http://api-demo.localhost/api/scrapers' : 'https://api.example.com/api/scrapers'),
    },

    customerio: {
      siteId: import.meta.env.VITE_CUSTOMERIO_SITE_ID || 'demo-site-id',
      apiKey: import.meta.env.VITE_CUSTOMERIO_API_KEY || 'demo-api-key',
      appApiKey: import.meta.env.CUSTOMERIO_APP_API_KEY,
    },

    auth: {
      storageKey: 'demo-admin-auth-token',
    },

    ui: {
      primaryColor: '#ca2b7f',  // Pink/Magenta - ML Ops brand color
      secondaryColor: '#4086c6',  // Blue - ML Ops secondary color
      logoPath: '/theme/demo/logo_white.png',  // Default to white logo
      faviconPath: '/theme/demo/',
    },
...
export function getBrandConfig(): BrandConfig {
  const brandId = import.meta.env.VITE_BRAND_ID || 'acme';
  const config = brands[brandId];

  if (!config) {
    console.warn(`Brand "${brandId}" not found, falling back to acme`);
    return brands.acme;
  }
...
export function getBrandId(): string {
  return import.meta.env.VITE_BRAND_ID || 'acme';
}
