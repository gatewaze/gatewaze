...
//
// All public reads route through `cdn.example.com` so cache hits stay at
// the edge and never touch the K8s API. Authenticated reads and writes
// route directly to `api.example.com` (the CDN explicitly bypasses on the
...
//     and uses the in-cluster service DNS (`http://example-api-svc:3002`).
