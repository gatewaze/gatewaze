...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `Upcoming - ${result.calendar.name} - ${brandConfig.name}`,
    description: result.calendar.description || `Upcoming events from ${result.calendar.name}`,
  }
}

export default async function CalendarUpcomingPage({ params }: Props) {
  const { identifier } = await params
  const brand = await getServerBrand()
  const brandConfig = await getBrandConfigById(brand)
  const result = await getCalendarWithEvents(brand, identifier)
