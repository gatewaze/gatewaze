...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `Calendar - ${result.calendar.name} - ${brandConfig.name}`,
    description: result.calendar.description || `Calendar view for ${result.calendar.name}`,
  }
}

export default async function CalendarGridPage({ params }: Props) {
  const { identifier } = await params
  const brand = await getServerBrand()
  const brandConfig = await getBrandConfigById(brand)
  const result = await getCalendarWithEvents(brand, identifier)
