...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `Past - ${result.calendar.name} - ${brandConfig.name}`,
    description: result.calendar.description || `Past events from ${result.calendar.name}`,
  }
}

export default async function CalendarPastPage({ params }: Props) {
  const { identifier } = await params
  const brand = await getServerBrand()
  const brandConfig = await getBrandConfigById(brand)
  const result = await getCalendarWithEvents(brand, identifier)
