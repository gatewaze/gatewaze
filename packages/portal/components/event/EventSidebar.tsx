...
interface NavItem {
   ...
   * and is never marked active by the local pathname comparison.
   */
  external?: boolean
}
...
function useNavItems(event: Event, basePath: string, speakerCount: number, sponsorCount: number, competitionCount: number, discountCount: number, mediaCount: number, hasVirtualEvent: boolean, userState?: EventUserState) {
  ...
  const navItems: NavItem[] = [
    ...
    {
      // Speak / Submit-a-talk item. Two paths:
      //   - external action_links.speak takes priority — links straight
      //     to the upstream CFP submission page (e.g. LF events'
      //     /speak/), bypassing the internal /talks form. The user
      //     hasn't been registered against our DB so /talks would be
      //     the wrong UX.
    ...
    // External action-link menu items, appended after the in-app pages
    // and before module-contributed pages. Each is conditionally shown
    // based on whether the scraper captured a URL for it. Distinct from
    // the in-app Sponsors / Agenda items above (which list our own DB
    // records) — these point at the upstream event's pages.
    {
      label: 'Schedule',
      href: event.source_details?.action_links?.schedule ?? '',
      external: true,
      show: !!event.source_details?.action_links?.schedule,
      icon: (
        <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    {
      label: 'Sponsor',
      href: event.source_details?.action_links?.sponsor ?? '',
      external: true,
      show: !!event.source_details?.action_links?.sponsor,
      icon: (
        <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    {
      label: 'Videos',
      href: event.source_details?.action_links?.videos ?? '',
      external: true,
      show: !!event.source_details?.action_links?.videos,
      icon: (
        <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
function useRegisterLink(event: Event, basePath: string) {
  ...
  // Scraper-captured Register button URL (when present) takes priority
  // over event_link. Concrete case: events.linuxfoundation.org cards
  // expose an explicit Register button that points at /register/, while
  // event_link points at the event's marketing landing page — sending
  // users to the marketing page when the user clicked Register would
  // bury the actual signup CTA.
  ...
  const registerHref = actionRegister
    ?? (useExternalLink ? event.event_link : `${basePath}/register`)
