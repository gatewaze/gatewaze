...
describe('recordUsage', () => {
  ...
  it('throws BudgetExceededError when over_hard', async () => {
    ...
    await expect(
      recordUsage(sb, {
        brand_id: 'example', provider: 'anthropic', product: 'x',
...
describe('callAnthropic', () => {
  it('records usage from the response.usage field', async () => {
    ...
    const result = await callAnthropic(
      sb,
      { brand_id: 'example', feature: 'test', model: 'claude-sonnet-4-6' },
...
describe('callOpenAI', () => {
  it('records usage from response.usage.prompt_tokens / completion_tokens', async () => {
    ...
    await callOpenAI(
      sb,
      { brand_id: 'example', feature: 'test', model: 'gpt-4o' },
