...
/**
 * Regenerate supabase/functions/platform-main/index.ts.
 *
 * ALL functions (core and module) use try/catch dynamic imports so that
 * a broken function cannot crash the router or take down other functions.
 * Failed functions return 503 with a descriptive error instead of 500.
 */
export function regeneratePlatformMain(functionsDir: string, moduleFunctionNames: string[]): void {
  ...
  // Extract existing core function names from previous generation
  // Matches both static imports and dynamic try/catch imports
  const staticImportRegex = /^import\s+\w+\s+from\s+'\.\.\/([^']+)\/index\.ts';$/gm;
  const dynamicImportRegex = /functions\['([^']+)'\]\s*=\s*\(await import/gm;
  const existingFunctions = new Set<string>();

  let match: RegExpExecArray | null;
  while ((match = staticImportRegex.exec(currentContent)) !== null) {
    existingFunctions.add(match[1]);
  }
  while ((match = dynamicImportRegex.exec(currentContent)) !== null) {
    existingFunctions.add(match[1]);
  }

  // Combine: existing functions + new module functions
  const allFunctions = new Set([...existingFunctions, ...moduleFunctionNames]);

  // Sort for deterministic output
  const sorted = [...allFunctions].sort();

  // Generate try/catch dynamic imports for ALL functions (core + module)
  const loadLines = sorted.map((fn) =>
    `  try { functions['${fn}'] = (await import('../${fn}/index.ts')).default; } catch (e) { loadErrors['${fn}'] = e?.message ?? String(e); }`
  );
...
`;
  ...
  console.log(`[modules] Regenerated platform-main with ${sorted.length} functions (all using resilient dynamic imports)`);
}
